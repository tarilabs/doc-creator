...
- Walks up to the RHAISTRAT parent and ensures a CCS Epic and Task exist
...
def find_or_create_ccs_epic(server, user, token, strat_key):
...
def process_mr(host, project, iid, mr_data, server, user, token, force=False):
    ...
    if strat_key:
        ...
        epic_key = find_or_create_ccs_epic(server, user, token, strat_key)
        try:
            add_labels(server, user, token, epic_key, [JIRA_LABEL])
            log.info("Added label '%s' to %s", JIRA_LABEL, epic_key)
        except Exception as e:
            log.warning("Failed to add label to %s: %s", epic_key, e)

        find_or_create_task(server, user, token, epic_key, mr_url, iid)
...
def main():
    ...
    if args.scan:
        ...
        # share the same RHAISTRAT parent (find_or_create_ccs_epic is not
